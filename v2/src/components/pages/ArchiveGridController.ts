...
export class ArchiveGridController {
  ...
  private getRequiredColumnHeight = () => {
    ...
    const tallestItemHeight = archive.reduce(
      (height, item) => Math.max(height, this.itemWidth * item.aspectRatio),
      0,
    );
    ...
    if (archive.length > 0) {
      ...
      while (
        this.colHeights.some(
          (columnHeight) => columnHeight < requiredColumnHeight,
        )
      ) {
  ...
  private loadImage = (src: string) => {
    ...
    const promise = new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      image.onload = () => {
        ...
        decode.catch(() => undefined).then(() => resolve(image));
      };
  ...
  private applyTransforms = () => {
    ...
    for (const tile of this.tiles) {
      const { placeholder, imageNode, baseX, baseY, width, height, colIndex } =
        tile;
      ...
      const hasMoved = tile.renderedX !== screenX || tile.renderedY !== screenY;
  ...
  private centerTile = (tile: Tile) => {
    ...
    const tileCenterX = tile.renderedX + (tile.width * tile.renderedScale) / 2;
    const tileCenterY = tile.renderedY + (tile.height * tile.renderedScale) / 2;
    const nextOffset = {
  ...
  private isTileCentered = (tile: Tile) => {
    ...
    return (
      Math.abs(tileCenterX - this.stage.width() / 2) <= OPEN_CENTER_THRESHOLD &&
      Math.abs(tileCenterY - this.stage.height() / 2) <= OPEN_CENTER_THRESHOLD
    );
  };
  ...
  private handlePinchMove = (event: TouchEvent) => {
    ...
    const nextScale = clamp(
      this.pinchStartScale * Math.pow(gestureRatio, TOUCH_PINCH_SENSITIVITY),
      this.minScale,
      MAX_SCALE,
    );
  ...
  private handlePointerMove = (
    ...
    const threshold =
      window.innerWidth < MOBILE_BREAKPOINT ? 24 : CLICK_MOVE_THRESHOLD;
    if (
      Math.hypot(
  ...
  private handleWheel = (event: Konva.KonvaEventObject<WheelEvent>) => {
    ...
    if (wheelEvent.ctrlKey) {
      ...
      this.targetScale = clamp(
        this.trackpadPinchStartScale *
          Math.exp(-this.accumulatedTrackpadPinchDelta * zoomSensitivity),
        this.minScale,
        MAX_SCALE,
      );
      this.zoomAnchorPoint = this.getPointerZoomPoint();
    ...
    const wheelX = wheelEvent.shiftKey ? deltaX || deltaY : deltaX;
