...
function useContextMenu(onOpen?: () => void) {
  ...
  useEffect(() => {
    ...
    const handleContextMenu = (e: MouseEvent) => {
      ...
      if (isTouchGeneratedContextMenu(e, lastTouchTimeRef.current)) {
        closeMenu();
        return;
      }
      ...
      if (card) {
        ...
        const mediaSrc =
          media.dataset.originalSrc || media.currentSrc || media.src;

        menuType = "image";
        menuData = {
          src: getOriginalImageSource(mediaSrc, window.location.href),
        };
      }
      ...
      setPos({
        ...
        y,
        alignX:
          x + MENU_WIDTH + HORIZONTAL_THRESHOLD > window.innerWidth
            ? "right"
            : "left",
...
export default function ContextMenu() {
  ...
  const { isOpen, openKey, pos, type, data, closeMenu } =
    useContextMenu(resetMenuInteraction);
  ...
  const handleCopyLink = useCallback(() => {
    if (!data.link) return;
    const url = data.link.startsWith("http")
      ? data.link
      ...
      if (keyboardScope === "submenu") {
        ...
        setSubmenuHoveredId((currentId) => {
          const currentIndex = currentId
            ? submenuItemIds.indexOf(currentId)
            : -1;
          const nextIndex =
            currentIndex === -1
              ? direction === 1
                ? 0
                : submenuItemIds.length - 1
              : (currentIndex + direction + submenuItemIds.length) %
      ...
      {isOpen && (
        <motion.div
          ...
          style={{
            ...
            right:
              pos.alignX === "right" ? window.innerWidth - pos.x : undefined,
            top: pos.alignY === "top" ? pos.y : undefined,
            bottom:
              pos.alignY === "bottom" ? window.innerHeight - pos.y : undefined,
          }}
          transition={{
            type: "spring",
            duration: 0.3,
              ...
              <AnimatePresence>
                {isSubMenuVisible && (
                  <motion.div
                    initial={{
                      opacity: 0,
                      x: pos.alignX === "left" ? -8 : 8,
                      scale: 0.98,
                    }}
                    animate={{ opacity: 1, x: 0, scale: 1 }}
                    exit={{
                      opacity: 0,
                      x: pos.alignX === "left" ? -4 : 4,
                      scale: 0.99,
                    }}
                    transition={{
                      type: "spring",
                  ...
                  >
                    <div
                      className={cn(
                        "absolute top-0 w-4 h-full cursor-pointer",
                        pos.alignX === "left" ? "-left-4" : "-right-4",
                      )}
                    />
                    <motion.div
                      className="relative isolate flex flex-col"
                    ...
                    >
                      <ContextMenuHighlight
                        selectedIndex={submenuHighlightIndex}
                      />
...
function ContextMenuItem({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative z-10 flex h-[34px] items-center gap-3 px-3 rounded-[8px] group",
        className,
      )}
      onClick={onClick}
