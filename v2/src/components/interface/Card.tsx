...
const isVideoUrl = (url: string) =>
  /\.(mp4|webm|ogg|mov|avi|ogv)(\?.*)?$/i.test(url);
...
function CardMedia({
  ...
  const mediaSrc = isCloudinaryVideo
    ...
    : image;
  const posterSrc = isCloudinaryVideo
    ? getCloudinaryVideoThumbnailSrc(image, imageWidth ?? 1200)
    : undefined;
  ...
  const containerAspectRatio =
    status === "loading"
      ? normalizedShimmerAspectRatio ||
        (isAuto ? DEFAULT_SHIMMER_ASPECT_RATIO : normalizedAspectRatio)
  ...
  const content = isAnyList ? (
    <div
      className={cn(
        ...
        isStacked
          ? "bg-[var(--background-primary)] rounded-t-xl"
          : "md:flex-row",
      )}
    >
      ...
      <div className="w-full flex flex-col gap-1" style={textOpacityStyle}>
        ...
        {description && (
          <div className="label-m text-[var(--content-secondary)]">
            {description}
          </div>
        )}
      </div>
    ...
    <div className="flex flex-col gap-2">
      ...
      {(title || description) && (
        ...
        >
          {title && (
            <div className="w-full text-[var(--content-primary)] label-m">
              {title}
            </div>
          )}
          {description && (
            <div className="w-full text-[var(--content-tertiary)] body-s-medium">
              {description}
            </div>
          )}
        </div>
