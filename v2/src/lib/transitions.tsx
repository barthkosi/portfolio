...
export const anim = new Proxy({} as any, {
  get(_, prop: string) {
    ...
    const dirKey = (Object.keys(directions).find((d) => key.includes(d)) ||
      "in") as keyof typeof directions;
    const physKey = (Object.keys(physics).find((p) => key.includes(p)) ||
      "standard") as keyof typeof physics;
...
export const Motion = ({
  ...
  return (
    <motion.div
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      transition={
        transition || { ...(variant.animate as any)?.transition, delay }
      }
      {...props}
    >
      {children}
