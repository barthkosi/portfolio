...
async function fetchImageDimensions(url) {
    ...
    if (!response.ok) {
        throw new Error(`Image fetch failed for "${url}" with status ${response.status}`);
    }

    const dimensions = getImageDimensionsFromBuffer(Buffer.from(await response.arrayBuffer()));
...
for (const dataPath of DATA_PATHS) {
    ...
    await writeFile(dataPath, `${JSON.stringify(enrichedItems, null, 4)}\n`, "utf8");
