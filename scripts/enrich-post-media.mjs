    ...
    assetParts = assetParts.filter((part) => {
      ...
      const isTransform =
        transformParams.some(
          (param) => part.startsWith(param) || part.includes("," + param),
        ) ||
        part.includes(",w_") ||
        part.includes(",h_") ||
        part.includes(",c_") ||
...
async function fetchAssetDimensions(url, existingMediaEntries) {
  ...
  try {
    ...
    if (isRateLimited(error)) {
      throw new Error(
        `Cloudinary Admin API rate limited and no cached metadata exists for "${url}"`,
      );
    }
  ...
  if (!asset.width || !asset.height) {
    throw new Error(
      `Missing dimensions for Cloudinary asset "${assetRef.publicId}"`,
    );
  }
...
async function main() {
  ...
  await writeFile(
    OUTPUT_PATH,
    `${JSON.stringify(mediaEntries, null, 4)}\n`,
    "utf8",
  );
